...
import LevelDBStorage from './leveldb-storage.mjs';
import { AnnouncementSerializer } from './serializers/announcement-serializer.mjs';
import { VRFProofSerializer } from './vrf/vrf-proof-serializer.mjs';
import { VRFNode } from './vrf/vrf-node.mjs';
import { Sha256Miner } from './miners/miner-sha.mjs';
...
class NodeManager {
  ...
  async createNode(nodeId, options) {
    try {
      ...
      const vrfProofSerializer = new VRFProofSerializer(1); 
      ...
      pubSubManager.registerMessageType('vrf-proof', new VRFProofMessageHandler(eventBus), vrfProofSerializer);
      ...
      switch (options.role) {
        case 'VRF_validator':
          node = new VRFNode(options, pubSubManager, blockManager, eventBus);
          break;
        case 'validator':
